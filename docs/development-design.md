...
- [ ] 用户可完成"开始专注 → 25 分钟 → 完成 → 获得燃料与 XP"闭环
- [ ] 等距地图可拖拽、缩放，帧率 PC ≥ 60fps、移动端 ≥ 30fps
- [ ] 可在已解锁区域放置至少 6 种建筑
- [ ] 至少 2 只猫咪可在地图上移动并响应点击对话
- [ ] 等级提升可解锁新建筑与新区域，迷雾正确揭开
- [ ] 刷新页面后进度完整保留
- [ ] PC 与移动端布局均可用
- [ ] 落地页滚动动画正常，无控制台报错
...
| 风险 | 影响 | 应对 |
| --- | --- | --- |
| PixiJS 与 React 集成复杂度 | 中 | 用 ref 挂载 PixiJS canvas，状态通过 Zustand 共享，避免 React 重渲染影响画布 |
| 等距瓦片美术资源缺失 | 高→中 | ✅ 已决策：MVP 阶段所有美术素材先用纯色/几何图形占位，后期用 AI 生成素材（GPT Image）置换。详见 [art-assets-requirements.md](./art-assets-requirements.md) |
| 移动端性能不足 | 中 | 提供画质降级开关，限制同屏对象数 |
| localStorage 存档丢失 | 中 | 提供导出/导入功能；迭代2上云同步 |
| 番茄钟后台计时被系统休眠中断 | 中 | 基于 `startedAt` 时间戳而非累计 tick，切回时用 `Date.now()` 校准 |

### 10.2 待定事项

- [x] ~~美术资源来源：自绘 / AI 生成 / 开源素材？~~ → 已决策：AI 生成（GPT Image），MVP 先用纯色占位，后期置换
- [ ] 音效来源：是否需要原创？
- [ ] 是否需要多语言（中/英）？
- [ ] 迭代2后端技术栈最终确认（SQLite vs PostgreSQL）
